...
from pkg_robot.srv import sum_two_int #Importamos el modulo SumaTwoInts generado por nuestro servicio 


def add_two_ints_client(x, y):               #Definimos una función para enviar la data al Service Server y obtener el resultado
    rospy.wait_for_service('sum_two_int')  #Esperamos el servicio si no está listo                                                                
    try:
        #Definimos el Servicio Cliente en la variable add_two_ints
                                          #Name Service|Clase Servicio
        add_two_ints = rospy.ServiceProxy('sum_two_int', sum_two_int)  
